...
describe('[ routes / user ]', () => {
    ...
    it('should not create a new user with existing email', async () => {
        // Arrage
        const user1 = { email: 'test@test.com', name: 'test', password: 'Test12345678!'}
    ...
    it('should not create a new user with empty name', async () => {
        // Arrage
        const userEmptyName = { email: 'test@test.com', name: '', password: 'Test12345678!'}
        // Act
...
describe('[routes / user/ login ]', () =>{
    ...
    it('should login a user with valid credentials', async () => {
        // Arrage
        const l_validUser = {email: 'claudia@gmail.com', password:'Password123?'}
        // Act
        const {status, body} = await request.post('/user/login').send(l_validUser)
        // Assert
    ...
    it('should NOT login a user with invalid email', async () => {
        // Arrage
        const l_Invaliduser = {email: 'tester@gmail.com', password:'Password123?'}
        // Act
        const {status, body} = await request.post('/user/login').send(l_Invaliduser)
        // Assert
    ...
    it('should NOT login a user with incompleted fields', async () => {
        ...
        // Act
        const {status, body} = await request.post('/user/login').send(l_Incompleteuser)
        // Assert
        expect(status).toEqual(400)
        expect(body.message).toEqual('Required fields')
    });
    ...
    it('should NOT login a user with invalid data types', async () => {
        ...
        // Act
        const {status, body} = await request.post('/user/login').send(l_InvalidDatauser)
        // Assert
        expect(status).toEqual(400)
        expect(body.message).toEqual('Invalid data type')
    });
    ...
    it('should NOT login a user with invalid email format', async () => {
        // Arrage
        const l_InvalidEmailuser = {email: '@gmail.com', password: 'Password123?'}
        // Act
        const {status, body} = await request.post('/user/login').send(l_InvalidEmailuser)
        // Assert
        expect(status).toEqual(400)
        expect(body.message).toEqual('Invalid email format')
    });
